...
class ResNet(nn.Module):
    def __init__(
        ...
        self.conv5 = conv1x1(512*block.expansion, 512)
        self.maxpool5 = nn.MaxPool2d((2, 2), (2, 2), (0, 1))
...
class CRNN_RNET(nn.Module):
    def __init__(self, nChannel=3, nHeight=32, nClass=128, nHidden=256, log_softmax=False):
        super(CRNN_RNET, self).__init__()

        self.log_softmax = log_softmax

        self.backbone = resnet34(nHeight=nHeight)
...
if __name__ == "__main__":
    nHeight = 64
    dummy = torch.randn(1, 3, nHeight, 200)
    crnn = CRNN_RNET(nHeight=nHeight)
    crnn(dummy)
